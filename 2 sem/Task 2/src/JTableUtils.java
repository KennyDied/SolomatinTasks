...
//                    }
    ...
    public static MyLinkedList readMyLinkedListToJTable(JTable table) {

        TableModel tableModel = table.getModel();
        int rowCount = tableModel.getRowCount(), colCount = tableModel.getColumnCount();
        MyLinkedList newArray = new MyLinkedList();
        for (int i = 0; i < rowCount; i++) {
            for (int j = 0; j < colCount; j++) {
                newArray.addLast(Double.parseDouble(tableModel.getValueAt(i, j).toString()));
            }
        }
        return newArray;
    }
